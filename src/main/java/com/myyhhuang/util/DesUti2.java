...
public class DesUti2 {
    ...
    public static void main(String[] args) {
        try {
            Map<String, Object> keyMap = DesUti2.initKeyPair();
            String publicKey = DesUti2.getPublicKey(keyMap);
            String privateKey = DesUti2.getPrivateKey(keyMap);
            System.out.println("公钥：" + publicKey);
            System.out.println("私钥：" + privateKey);
            String source = "我是需要私钥加密的字符串！";
            System.out.println("签名验证逻辑，私钥加密--公钥解密，需要加密的字符串：" + source);
            byte[] data = source.getBytes();
            byte[] encodedData = DesUti2.encryptByPrivateKey(data, privateKey);
            System.out.println("私钥加密后：" + new String(encodedData));
            String sign = DesUti2.sign(encodedData, privateKey);
            System.out.println("签名:" + sign);
            boolean status = DesUti2.verify(encodedData, publicKey, sign);
            System.out.println("验证结果:" + status);
            byte[] decodedData = DesUti2.decryptByPublicKey(encodedData, publicKey);
            String target = new String(decodedData);
            System.out.println("公钥解密私钥加密的数据:" + target);

            System.out.println("---------公钥加密----私钥解密----------");
            // 这里尽量长一点，复制了一段歌词
            String msg = "月溅星河，长路漫漫，风烟残尽，独影阑珊";
            String ecodeMsg = DesUti2.encryptedDataByPublic(msg,publicKey);
            System.out.println("加密后的歌词：" + ecodeMsg);
            String decodeMsg = DesUti2.decryptDataByPrivate(ecodeMsg,privateKey);
